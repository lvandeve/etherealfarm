...
function blendImages(a, b) {
  ...
  var result = [r, w, h];
  // If the input objects were images on canvas (the format returned by setupImage instead of generateImage), put the result also in that same format
  if(a.length > 3) result = setupImage(result);
  return result;
}
